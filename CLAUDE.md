...
- Limite noto non ancora risolto: non essendoci un modo per distinguere "Google collegato davvero" dal companion automatico, il bottone "Collega account" in Impostazioni resta visibile anche dopo un vero collegamento Google riuscito da un profilo Play Games (finché non si collega anche Facebook) — è solo un invito ridondante, non causa perdita dati.
...
Sospetto principale, non confermato: gestione batteria/processi molto aggressiva lato Motorola, osservata nei log (`moto_freezer`, `MotoBatteryCareService`) per **molte app diverse**, non solo TurtleMind — potrebbe interferire in modo non deterministico con la consegna (ha funzionato in test precedenti, non in questo). Prossimi passi per la prossima sessione: verificare le impostazioni di ottimizzazione batteria per l'app sul device di test (Impostazioni → Batteria → TurtleMind → nessuna restrizione), e se possibile ripetere il test su un device non Motorola per isolare la variabile.
