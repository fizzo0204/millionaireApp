...
- **Bug reale trovato e corretto (2026-08-18, sera)**: la nota "verificato su device reale" più sotto (stessa giornata, test precedente) si è rivelata una falsa verifica. `shouldLinkCurrentProfileToProvider()` decideva se tentare `linkWithCredential` controllando `userHasProvider(user, 'google.com')` **senza** lo stesso trattamento speciale del companion applicato in `isBaseProfile()`: dato che `google.com` è sempre già presente su un profilo Play Games, il controllo restituiva "già collegato" e il codice **saltava del tutto** `linkWithCredential`, cadendo nel ramo `else` di `googleSignIn()` che fa un `signInWithCredential` pieno — capace di creare/caricare un account Google con un **uid diverso**, perdendo i progressi Play Games. Riprodotto su device reale ripulendo prima tutti gli account Firebase di test (così nessun profilo pregresso mascherava il problema prendendo il branch `credential-already-in-use` invece di quello reale): monete scese dal valore atteso a `USER_STATS_CONFIG.defaultCoins` (20) e daily reward tornato "mai riscattato". Corretto: `shouldLinkCurrentProfileToProvider()` ora forza sempre il tentativo di link quando `providerId` è Google e l'utente ha già Play Games, lasciando che sia l'esito di `linkWithCredential` (`auth/provider-already-linked` vs `auth/credential-already-in-use`) a decidere il resto — la gestione di quei due codici in `googleSignIn()` era già corretta, semplicemente non veniva mai raggiunta. La nota precedente ("verificato su device reale") attribuiva il mancato ripetersi del problema al fatto che l'account Google di test avesse probabilmente un profilo pregresso: era la spiegazione giusta, ma significava che il percorso `provider-already-linked` non era mai stato davvero esercitato — non una vera verifica.
