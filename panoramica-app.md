...
1. **AdMob è configurato con gli ID di test pubblici di Google** (sia in `ads.config.ts` che nell'`AndroidManifest.xml`, incluso l'Application ID `ca-app-pub-3940256099942544~...`). Serve: creare/collegare un account AdMob reale all'app, generare gli ID veri (banner + rewarded + Application ID) e sostituirli in quei due file. Senza questo, niente ricavi pubblicitari.
2. **Manca una privacy policy** collegata all'app (né in-app né nella scheda Play Store attesa). Serve: scrivere/generare una privacy policy che copra AdMob, Facebook Login e Firebase, pubblicarla su un URL raggiungibile, e linkarla sia nella scheda Play Store sia da qualche parte in-app (es. Impostazioni).
3. **Manca la gestione del consenso GDPR/pubblicità** (Google User Messaging Platform) per utenti UE/UK: obbligatoria per gli editori AdMob che servono utenti europei. Dipende dal punto 1 (serve l'account AdMob reale prima di poter configurare il consenso in console).
4. **Certificato di firma release non ancora registrato su Firebase**: `google-services.json` contiene solo l'impronta SHA-1 di debug. Serve: generare/recuperare il keystore di release, prendere il suo SHA-1 (e quello di Play App Signing una volta caricata la prima build su Play Console), e aggiungerli entrambi all'app Android nella console Firebase. Senza questo, login Google/Facebook/Play Games smettono di funzionare nella build pubblicata.
5. **Build di release non minificata** (`minifyEnabled false` in `android/app/build.gradle`, nonostante esistano già regole ProGuard pronte in `proguard-rules.pro`): da valutare l'attivazione di R8/ProGuard per dimensione e protezione minima del codice. Serve testarla su una build di release reale e firmata (Facebook/Google Sign-In a volte si rompono senza regole `-keep` dedicate) — non ha senso farlo prima di avere un keystore di release (vedi punto 4).
