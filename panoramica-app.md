...
Punti risolti:
- **Keystore di release configurato (2026-08-10)**: `google-services.json` ora include anche la SHA-1 di release (oltre a quella di debug). Aggiunto `signingConfigs.release` in `android/app/build.gradle`, che legge le credenziali da `android/keystore.properties` (non tracciato da Git — vedi `android/keystore.properties.example` per il formato). Il keystore vive volutamente fuori dal repo, in `C:\Project\turtlemind-release.keystore`. Verificato con `./gradlew :app:signingReport` che il variant `release` punta correttamente a quel file e all'alias `turtlemind`; manca solo la password reale (lasciata vuota di proposito, va inserita a mano in locale) perché la firma funzioni davvero. `minifyEnabled` resta `false` di proposito, affrontato separatamente (vedi punto 2 sopra).
